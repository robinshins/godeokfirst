...
const consultationSchema = z.object({
  ...
  contactType: z.enum(['phone', 'email', 'instagram'], {
    required_error: "Iltimos, bog'lanish usulini tanlang"
  }),
  contact: z.string().min(1, "Iltimos, bog'lanish ma'lumotlarini kiriting"),
  age: z.number().min(1, 'Iltimos, yoshingizni kiriting').max(120, "Iltimos, to'g'ri yoshni kiriting"),
  consultationContent: z.string().min(1, 'Iltimos, maslahat mavzusini tanlang')
});
...
const presetOptions = [
  { main: 'Implant', sub: '' },
  { main: 'Nerv davolash', sub: '' },
  { main: 'Tish qoplash', sub: 'Toj' },
  { main: 'Tish oqartirish', sub: 'Laminat' },
  { main: 'Aql tishi', sub: '' },
  { main: 'Milk parvarishi', sub: '' },
  { main: 'Kariyes', sub: 'Rezin, Inlay' },
  { main: 'Jag\' bo\'g\'imi', sub: '' }
];
...
export default function ConsultationForm() {
  ...
  const getContactPlaceholder = () => {
    switch (contactType) {
      case 'phone':
        return '10-1234-5678';
      case 'email':
        return 'sizning@email.com';
      case 'instagram':
        return '@sizning_instagram';
      default:
        return '';
    }
  };
  ...
  const proceedToChat = async (data: ConsultationFormData) => {
    ...
    try {
      ...
      // Supabase에 즉시 저장
      console.log('Maslahat so\'rovi yuborildi - Supabase\'ga saqlash boshlandi');
      try {
        const logData = {
          name: data.name,
          phone: data.contact,
        ...
        if (response.ok) {
          ...
          console.log('Maslahat jurnali yaratildi, ID:', logId);

          if (logId) {
            localStorage.setItem('consultationLogId', logId);
            console.log('Jurnal ID localStorage\'ga saqlandi:', logId);
            ...
            if (typeof window !== 'undefined') {
              ...
              if (fbq) {
                fbq('track', 'Lead');
                console.log('Meta Pixel: Lead hodisasi yuborildi');
              }
              ...
              if (ttq && typeof ttq.track === 'function') {
                ...
                console.log('TikTok Pixel: SubmitForm hodisasi yuborildi');
              }
            }
          }
        } else {
          console.error('Maslahat jurnali yaratilmadi:', response.status, await response.text());
        }
      } catch (error) {
        console.error('Maslahat jurnali yaratishda xatolik:', error);
      }
      ...
      console.error('Maslahat so\'rovida xatolik:', error);
      setShowLoadingOverlay(false);
    }
  };
    ...
    <div className="bg-white w-full px-4 py-[50px] mb-[50px]">
      <div className="max-w-[375px] mx-auto">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[30px]">
          ...
          <div className="flex flex-col gap-[30px]">
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Ism<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('name')}
                type="text"
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Bog'lanish usuli<span className="text-[#ff1616]">*</span>
              </label>
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              {contactType === 'instagram' && (
                <p className="text-xs text-[#727582] mt-1">
                  Iltimos, Instagram ID kiriting (masalan, @username)
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Yosh<span className="text-[#ff1616]">*</span>
              </label>
              <input
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              />
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Maslahat mavzusi<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex flex-col gap-[6px]">
                {/* Row 1 */}
          ...
          >
            {isSubmitting ? 'Yuborilmoqda...' : 'AI maslahatni boshlash'}
          </button>
        </form>
      </div>
          ...
          <div className="bg-white rounded-[24px] p-6 max-w-[340px] w-full">
            <div className="text-center">
              ...
              <p className="font-['Pretendard'] font-normal text-[12px] text-[#727582] tracking-[-0.24px]">
                AI maslahatga yo'naltirilmoqda...
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="font-['Pretendard'] font-semibold text-[16px] text-[#292a2f] tracking-[-0.32px] mb-2">
                AI maslahatchisi tayyorlanmoqda
              </p>
