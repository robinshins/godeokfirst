...
export default function GNB() {
  ...
  useEffect(() => {
    // Используется только onAuthStateChange (Удалён вызов getUser для экономии egress)
    // Акция INITIAL_SESSION предоставляет текущее состояние сессии
  ...
  const languages = [
    { code: 'kr', name: 'Корейский', flag: '/icons/flags-kr.svg', href: getCurrentPage() === '/' ? '/' : getCurrentPage() },
  ...
  const menuItems = [
    ...
    { name: 'Протезы · Полная имплантация', href: '/denture' },
    { name: 'Лечение кариеса · Эндодонтия', href: '/cavity-treatment' },
    { name: 'Удаление зубов мудрости', href: '/wisdom-tooth' },
    { name: 'Лечение десен · Чистка', href: '/gum-care' },
    { name: 'Лечение ВНЧС', href: '/tmj' },
    { name: 'Отбеливание зубов', href: '/whitening' },
  ];
  ...
  return (
    ...
    >
      ...
      >
        {/* Logo */}
        <Link href="/ru" className="flex items-center" style={{ height: '60px' }}>
          <Image
