...
const IMPLANT_GUIDE: BlogTopicConfig = {
  ...
  metaDescriptionFn: (r) =>
    `${r} 임플란트 치료를 고민하시는 분들을 위한 완벽 가이드. 수술 과정, 뼈이식, 보철 재료, 성공률, 수명, 사후관리까지 통합치의학전문의가 의학 기반으로 설명합니다.`,
  keywordsFn: (r) => [
    `${r} 임플란트`,
    `${r} 임플란트 가이드`,
    `${r} 임플란트 치과`,
...
const WISDOM_TOOTH_GUIDE: BlogTopicConfig = {
  ...
  metaDescriptionFn: (r) =>
    `${r} 사랑니 발치를 고민 중인 분들을 위한 완벽 가이드. 매복 사랑니 분류, 신경 손상 위험, 건조소켓, 발치 후 관리까지 대학병원 외래교수 출신 원장이 정리했습니다.`,
  keywordsFn: (r) => [
    `${r} 사랑니`,
    `${r} 사랑니발치`,
    `${r} 매복 사랑니`,
