...
const consultationSchema = z.object({
  ...
  contactType: z.enum(['phone', 'email', 'instagram'], {
    required_error: '请选择联系方式'
  }),
  contact: z.string().min(1, '请输入您的联系信息'),
  age: z.number().min(1, '请输入您的年龄').max(120, '请输入有效年龄'),
  consultationContent: z.string().min(1, '请选择咨询主题')
});
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[30px]">
          ...
          <div className="flex flex-col gap-[30px]">
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard',sans-serif] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                姓名<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('name')}
                type="text"
                placeholder="请输入您的姓名"
                className={`w-full bg-white border ${errors.name ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Pretendard',sans-serif] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
              />
              {errors.name && (
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard',sans-serif] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                联系方式<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => handleContactTypeChange('phone')}
                  className={`flex-1 py-3 rounded-[12px] font-['Pretendard',sans-serif] font-semibold text-[14px] tracking-[-0.28px] transition-colors ${
                    contactType === 'phone'
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              {contactType === 'phone' ? (
                <div className={`flex items-center w-full bg-white border ${errors.contact ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 focus-within:border-[#006aff]`}>
                  <span className="font-['Pretendard',sans-serif] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] text-black pr-2">
                    +82
                  </span>
                  <input
                    {...register('contact')}
                    ...
                    className="flex-1 bg-transparent py-[17px] font-['Pretendard',sans-serif] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none"
                  />
                </div>
              ) : (
                <input
                  ...
                  type="text"
                  placeholder={getContactPlaceholder()}
                  className={`w-full bg-white border ${errors.contact ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Pretendard',sans-serif] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
                />
              )}
              {errors.contact && (
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard',sans-serif] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                年龄<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('age', { valueAsNumber: true })}
                type="number"
                placeholder="0"
                className={`w-full bg-white border ${errors.age ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Pretendard',sans-serif] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
              />
              {errors.age && (
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              />
              <label className="font-['Pretendard',sans-serif] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                咨询主题<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex flex-col gap-[6px]">
                {/* Row 1 */}
                <div className="flex gap-[7px]">
                  {presetOptions.slice(0, 3).map((option, index) => (
                    ...
                    >
                      <p className="font-['Pretendard',sans-serif] font-semibold text-[15px] text-black tracking-[-0.3px] leading-[1.4]">
                        {option.main}
                      </p>
                      {option.sub && (
                        <p className="font-['Pretendard',sans-serif] font-normal text-[12px] text-[#5d5f6d] tracking-[-0.24px] leading-[1.4]">
                          {option.sub}
                        </p>
                      )}
                    </button>
          ...
          <button
            ...
            className="w-full bg-[#006aff] rounded-[16px] py-4 font-['Pretendard',sans-serif] font-bold text-[18px] text-white tracking-[-0.36px] leading-[1.5] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#0058d6] transition-colors"
          >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] p-6 max-w-[340px] w-full">
            <div className="text-center">
              ...
              <h3 className="font-['Pretendard',sans-serif] font-bold text-[20px] text-black tracking-[-0.4px] mb-2">
                请给我们发私信！
              </h3>
              <p className="font-['Pretendard',sans-serif] font-medium text-[14px] text-[#5d5f6d] tracking-[-0.28px] leading-[1.6] mb-4">
                为了确保我们的回复不会被遗漏，请给我们发送私信：
              </p>
              <a
                href="https://www.instagram.com/dr_kimplant/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 text-white font-['Pretendard',sans-serif] font-bold text-[16px] tracking-[-0.32px] px-6 py-3 rounded-[12px] mb-4 hover:opacity-90 transition-opacity"
              >
                @dr_kimplant
              </a>
              <p className="font-['Pretendard',sans-serif] font-normal text-[12px] text-[#727582] tracking-[-0.24px]">
                正在跳转到AI咨询...
              </p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-white z-50 flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="font-['Pretendard',sans-serif] font-semibold text-[16px] text-[#292a2f] tracking-[-0.32px] mb-2">
                AI咨询员准备中
              </p>
              <p className="font-['Pretendard',sans-serif] font-normal text-[14px] text-[#727582] tracking-[-0.28px]">
                请稍候...
              </p>
