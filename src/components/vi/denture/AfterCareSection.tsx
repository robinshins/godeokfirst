import Image from 'next/image';

export default function AfterCareSection() {
  const careItems = [
    {
      icon: '🔍',
      title: 'Hệ thống kiểm tra định kỳ',
      desc: 'Kiểm tra chính xác theo chu kỳ 3 tháng, 6 tháng, 1 năm sau cấy Implant để phát hiện sớm bất thường.',
    },
    {
      icon: '🦷',
      title: 'Vệ sinh quanh Implant',
      desc: 'Loại bỏ màng vi khuẩn quanh Implant bằng thiết bị chuyên dụng và chăm sóc phòng ngừa viêm quanh Implant.',
    },
    {
      icon: '📋',
      title: 'Kế hoạch chăm sóc cá nhân hóa',
      desc: 'Lập kế hoạch chăm sóc cá nhân theo tình trạng miệng và hướng dẫn đánh răng đúng cách và sử dụng chỉ nha khoa.',
    },
  ];

  const steps = [
    { step: '1', title: 'Kiểm tra định kỳ', desc: 'Chu kỳ 3·6·12 tháng' },
    { step: '2', title: 'Vệ sinh chuyên nghiệp', desc: 'Phòng ngừa viêm quanh' },
    { step: '3', title: 'Kiểm tra khớp cắn', desc: 'Kiểm tra cân bằng lực nhai' },
    { step: '4', title: 'Chăm sóc suốt đời', desc: 'Trách nhiệm sau liên tục' },
  ];

  return (
    <div className="bg-[#0b1727] w-full flex justify-center">
      <div className="box-border flex flex-col gap-[50px] items-center px-4 py-[60px] relative w-full max-w-[430px]">
        {/* Header */}
        <div className="flex flex-col gap-4 items-center justify-center relative shrink-0 w-full">
          <div className="h-8 overflow-clip relative shrink-0 w-[180px]">
            <Image
              src="/고덕퍼스트치과-로고-(화트).png"
              alt="Godeok First Dental"
              width={375}
              height={84}
              className="h-20 w-auto object-contain -mt-[18px]"
            />
          </div>
          <div className="flex flex-col gap-4 items-center justify-center relative shrink-0 w-full">
            <p className="text-[14px] font-bold text-[#4DC8D8] tracking-[3px] uppercase">
              AFTER CARE
            </p>
            <h2 className="font-bold leading-[1.35] not-italic text-[32px] text-white text-center tracking-[-0.64px] w-full">
              Điều quan trọng hơn cấy ghép là<br />
              chăm sóc sau điều trị
            </h2>
            <p className="font-medium leading-[1.6] not-italic text-[#a0c4d4] text-[15px] text-center tracking-[-0.32px] w-full">
              &quot;Để bạn có thể sử dụng suốt đời<br />
              chúng tôi chịu trách nhiệm đến cùng&quot;
            </p>
          </div>
        </div>

        {/* Hạng mục chăm sóc sau */}
        <div className="flex flex-col gap-4 w-full">
          {careItems.map((item, i) => (
            <div key={i} className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#008095]/20 flex items-center justify-center shrink-0 text-2xl">
                  {item.icon}
                </div>
                <div className="flex-1">
                  <p className="font-bold text-white text-[16px] mb-1">{item.title}</p>
                  <p className="text-[#a0c4d4] text-[13px] leading-[1.6]">{item.desc}</p>
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* Ảnh phòng phẫu thuật */}
        <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden">
          <Image
            src="/images/goduk_images/수술실.jpg"
            alt="Phòng phẫu thuật Godeok First Dental"
            fill
            className="object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
          <div className="absolute bottom-4 left-4 right-4">
            <p className="text-white font-bold text-[15px]">Môi trường phẫu thuật sạch sẽ và an toàn</p>
            <p className="text-white/80 text-[13px]">Hệ thống kiểm soát nhiễm trùng cấp bệnh viện đại học</p>
          </div>
        </div>

        {/* Quy trình 4 bước */}
        <div className="w-full">
          <p className="text-[#4DC8D8] text-[13px] font-bold mb-5 text-center tracking-[2px]">
            Quy trình chăm sóc sau của Godeok First Dental
          </p>
          <div className="grid grid-cols-4 gap-2">
            {steps.map((s) => (
              <div key={s.step} className="flex flex-col items-center gap-2">
                <div className="bg-[#008095] rounded-full w-11 h-11 flex items-center justify-center">
                  <span className="text-white font-black text-[16px]">{s.step}</span>
                </div>
                <p className="text-white text-[12px] font-bold text-center">{s.title}</p>
                <p className="text-[#a0c4d4] text-[11px] text-center leading-[1.3]">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>

        {/* Thông điệp nhấn mạnh */}
        <div className="bg-gradient-to-r from-[#008095] to-[#006B7A] rounded-2xl p-5 w-full text-center shadow-lg">
          <p className="text-white font-bold text-[16px] leading-[1.5]">
            Sau Phẫu thuật cấy ghép Implant<br />
            Bác sĩ trưởng trực tiếp chịu trách nhiệm đến cùng
          </p>
        </div>
      </div>
    </div>
  );
}
