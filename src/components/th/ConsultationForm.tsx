...
const consultationSchema = z.object({
  ...
  contact: z.string().min(1, 'กรุณาใส่ข้อมูลติดต่อ'),
  age: z.number().min(1, 'กรุณาใส่อายุ').max(120, 'กรุณาใส่อายุที่ถูกต้อง'),
  consultationContent: z.string().min(1, 'กรุณาเลือกหัวข้อปรึกษา')
});

type ConsultationFormData = z.infer<typeof consultationSchema>;

const presetOptions = [
  { main: 'รากฟันเทียม', sub: '' },
  { main: 'รักษารากฟัน', sub: '' },
  { main: 'ครอบฟัน', sub: '' },
  { main: 'ฟอกสีฟัน', sub: 'ลามิเนต' },
  { main: 'ฟันคุด', sub: '' },
  { main: 'ดูแลเหงือก', sub: '' },
  { main: 'อุดฟัน', sub: 'เรซิ่น, อินเลย์' },
  { main: 'ข้อต่อขากรรไกร', sub: '' }
];
...
export default function ConsultationForm() {
  ...
  const proceedToChat = async (data: ConsultationFormData) => {
    ...
    try {
      ...
      // Supabase에 즉시 저장
      console.log('Submitting consultation form - saving to Supabase');
      try {
        const logData = {
          name: data.name,
          phone: data.contact,
        ...
        if (response.ok) {
          ...
          console.log('Consultation log created, ID:', logId);

          if (logId) {
            localStorage.setItem('consultationLogId', logId);
            console.log('Log ID saved to localStorage:', logId);
            ...
            if (typeof window !== 'undefined') {
              ...
              if (ttq && typeof ttq.track === 'function') {
                ...
                console.log('TikTok Pixel: SubmitForm event sent');
              }
            }
          }
        } else {
          console.error('Failed to create consultation log:', response.status, await response.text());
        }
      } catch (error) {
    ...
    <div className="bg-white w-full px-4 py-[50px] mb-[50px]">
      <div className="max-w-[375px] mx-auto">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[30px]">
          ...
          <div className="flex flex-col gap-[30px]">
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                ชื่อ<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('name')}
                type="text"
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                วิธีการติดต่อ<span className="text-[#ff1616]">*</span>
              </label>
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              {contactType === 'instagram' && (
                <p className="text-xs text-[#727582] mt-1">
                  กรุณาใส่ Instagram ID ของคุณ (เช่น @username)
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                อายุ<span className="text-[#ff1616]">*</span>
              </label>
              <input
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              />
              <label className="font-['Pretendard'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                คุณต้องการปรึกษาเรื่องอะไร?<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex flex-col gap-[6px]">
                {/* Row 1 */}
          ...
          >
            {isSubmitting ? 'กำลังส่ง...' : 'เริ่มปรึกษากับ AI'}
          </button>
        </form>
      </div>
          ...
          <div className="bg-white rounded-[24px] p-6 max-w-[340px] w-full">
            <div className="text-center">
              ...
              <p className="font-['Pretendard'] font-medium text-[14px] text-[#5d5f6d] tracking-[-0.28px] leading-[1.6] mb-4">
                เพื่อให้แน่ใจว่าข้อความตอบกลับจะไม่สูญหาย กรุณาส่ง DM มาที่:
              </p>
              <a
              ...
              <p className="font-['Pretendard'] font-normal text-[12px] text-[#727582] tracking-[-0.24px]">
                กำลังเปลี่ยนเส้นทางไปยังการปรึกษา AI...
              </p>
            </div>
          </div>
        </div>
