...
const consultationSchema = z.object({
  ...
  contactType: z.enum(['phone', 'email', 'instagram'], {
    required_error: 'Холбоо барих аргаа сонгоно уу'
  }),
  contact: z.string().min(1, 'Холбоо барих мэдээллээ оруулна уу'),
  age: z.number().min(1, 'Насаа оруулна уу').max(120, 'Зөв нас оруулна уу'),
  consultationContent: z.string().min(1, 'Зөвлөгөөний сэдэв сонгоно уу')
});
...
const presetOptions = [
  { main: 'Имплант', sub: '' },
  { main: 'Мэдрэлийн эмчилгээ', sub: '' },
  { main: 'Шүд бүрэх', sub: 'Титэм' },
  { main: 'Гоо сайхан', sub: 'Цайруулах, Ламинат' },
  { main: 'Сүүлийн шүд', sub: '' },
  { main: 'Буйлны эмчилгээ', sub: '' },
  { main: 'Цоорхой', sub: 'Резин, Инлэй' },
  { main: 'Эрүү үений эмгэг', sub: '' }
];
...
export default function ConsultationForm() {
  ...
  const proceedToChat = async (data: ConsultationFormData) => {
    ...
    try {
      ...
      // Supabase에 즉시 저장
      console.log('Зөвлөгөөний хүсэлт илгээж байна - Supabase руу хадгалж байна');
      try {
        const logData = {
          name: data.name,
          phone: data.contact,
        ...
        if (response.ok) {
          ...
          console.log('Зөвлөгөөний лог үүсгэгдлээ, ID:', logId);

          if (logId) {
            localStorage.setItem('consultationLogId', logId);
            console.log('Лог ID localStorage-д хадгалагдлаа:', logId);
            ...
            if (typeof window !== 'undefined') {
              ...
              if (fbq) {
                fbq('track', 'Lead');
                console.log('Meta Pixel: Lead event илгээгдлээ');
              }
              ...
              if (ttq && typeof ttq.track === 'function') {
                ...
                console.log('TikTok Pixel: SubmitForm event илгээгдлээ');
              }
            }
          }
        } else {
          console.error('Зөвлөгөөний лог үүсгэхэд алдаа гарлаа:', response.status, await response.text());
        }
      } catch (error) {
        console.error('Зөвлөгөөний лог үүсгэхэд алдаа гарлаа:', error);
      }
      ...
      console.error('Зөвлөгөө хүсэхэд алдаа гарлаа:', error);
      setShowLoadingOverlay(false);
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[30px]">
          ...
          <div className="flex flex-col gap-[30px]">
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Noto_Sans'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Нэр<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('name')}
                type="text"
                placeholder="Нэрээ оруулна уу"
                className={`w-full bg-white border ${errors.name ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Noto_Sans'] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
              />
              {errors.name && (
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Noto_Sans'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Холбоо барих арга<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => handleContactTypeChange('phone')}
                  className={`flex-1 py-3 rounded-[12px] font-['Noto_Sans'] font-semibold text-[14px] tracking-[-0.28px] transition-colors ${
                    contactType === 'phone'
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              {contactType === 'phone' ? (
                <div className={`flex items-center w-full bg-white border ${errors.contact ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 focus-within:border-[#006aff]`}>
                  <span className="font-['Noto_Sans'] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] text-black pr-2">
                    +82
                  </span>
                  <input
                    {...register('contact')}
                    ...
                    className="flex-1 bg-transparent py-[17px] font-['Noto_Sans'] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none"
                  />
                </div>
              ) : (
                <input
                  ...
                  type="text"
                  placeholder={getContactPlaceholder()}
                  className={`w-full bg-white border ${errors.contact ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Noto_Sans'] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
                />
              )}
              {errors.contact && (
              ...
              {contactType === 'instagram' && (
                <p className="text-xs text-[#727582] mt-1">
                  Instagram ID-аа оруулна уу (жишээ нь: @username)
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-[6px]">
              <label className="font-['Noto_Sans'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Нас<span className="text-[#ff1616]">*</span>
              </label>
              <input
                {...register('age', { valueAsNumber: true })}
                type="number"
                placeholder="0"
                className={`w-full bg-white border ${errors.age ? 'border-red-400' : 'border-[#e9ebf1]'} rounded-[16px] px-4 py-[17px] font-['Noto_Sans'] font-semibold text-[16px] tracking-[-0.32px] leading-[1.4] placeholder:text-[#aab2c1] focus:outline-none focus:border-[#006aff]`}
              />
              {errors.age && (
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              />
              <label className="font-['Noto_Sans'] font-semibold text-[13px] text-[#727582] tracking-[-0.26px] leading-[1.4]">
                Та юуны талаар зөвлөгөө авахыг хүсч байна вэ?<span className="text-[#ff1616]">*</span>
              </label>
              <div className="flex flex-col gap-[6px]">
                {/* Row 1 */}
                <div className="flex gap-[7px]">
                  {presetOptions.slice(0, 3).map((option, index) => (
                    ...
                    >
                      <p className="font-['Noto_Sans'] font-semibold text-[15px] text-black tracking-[-0.3px] leading-[1.4]">
                        {option.main}
                      </p>
                      {option.sub && (
                        <p className="font-['Noto_Sans'] font-normal text-[12px] text-[#5d5f6d] tracking-[-0.24px] leading-[1.4]">
                          {option.sub}
                        </p>
                      )}
                    </button>
          ...
          <button
            ...
            className="w-full bg-[#006aff] rounded-[16px] py-4 font-['Noto_Sans'] font-bold text-[18px] text-white tracking-[-0.36px] leading-[1.5] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#0058d6] transition-colors"
          >
            {isSubmitting ? 'Илгээж байна...' : 'AI зөвлөгөө эхлүүлэх'}
          </button>
        </form>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] p-6 max-w-[340px] w-full">
            <div className="text-center">
              ...
              <h3 className="font-['Noto_Sans'] font-bold text-[20px] text-black tracking-[-0.4px] mb-2">
                Манайд DM илгээнэ үү!
              </h3>
              <p className="font-['Noto_Sans'] font-medium text-[14px] text-[#5d5f6d] tracking-[-0.28px] leading-[1.6] mb-4">
                Манай хариулт алга болохгүйн тулд дараах хаягаар DM илгээнэ үү:
              </p>
              <a
                href="https://www.instagram.com/dr_kimplant/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 text-white font-['Noto_Sans'] font-bold text-[16px] tracking-[-0.32px] px-6 py-3 rounded-[12px] mb-4 hover:opacity-90 transition-opacity"
              >
                @dr_kimplant
              </a>
              <p className="font-['Noto_Sans'] font-normal text-[12px] text-[#727582] tracking-[-0.24px]">
                AI зөвлөгөө рүү шилжиж байна...
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-white z-50 flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="font-['Noto_Sans'] font-semibold text-[16px] text-[#292a2f] tracking-[-0.32px] mb-2">
                AI зөвлөгч бэлтгэж байна
              </p>
              <p className="font-['Noto_Sans'] font-normal text-[14px] text-[#727582] tracking-[-0.28px]">
                Түр хүлээнэ үү...
              </p>
