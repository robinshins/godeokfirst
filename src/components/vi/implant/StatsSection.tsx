...
export default function StatsSection() {
  const stats = [
    { number: '1,200', unit: 'lần+', label: 'Số ca cấy ghép Implant' },
    { number: '0', unit: 'ca', label: 'Phẫu thuật lại Implant', subtitle: 'Tái phẫu thuật từ khi khai trương Godeok Edutown 0ca' },
    { number: '4.9', unit: '/ 5.0', label: 'Sự hài lòng của bệnh nhân', subtitle: 'Dựa trên Kakao, Google, Naver Map' },
  ];
      ...
      <FadeInSection className="flex flex-col gap-4 items-center justify-center relative shrink-0 w-full z-10">
        <div className="flex flex-col gap-3 items-center justify-center relative shrink-0 w-full">
          ...
          <p className="font-semibold leading-[1.4] not-italic text-[#e9ebf1] text-base text-center tracking-[-0.32px] w-full" style={{ fontFamily: '"NanumSquare", sans-serif' }}>
            Giáo sư thỉnh giảng BV Nha khoa Đại học Chosun xuất thân<br />
            Bác sĩ Lee Dong-hyun trực tiếp phẫu thuật
          </p>
        </div>
      </FadeInSection>
        ...
        <div className="flex flex-col gap-4 items-center justify-center relative shrink-0 w-full">
          {stats.map((stat, index) => (
            <FadeInSection key={index} delay={index * 150} className="bg-white box-border flex flex-col gap-4 items-center px-4 py-10 relative rounded-2xl shrink-0 w-full">
              {/* SEO: Văn bản ẩn để công cụ tìm kiếm đọc */}
