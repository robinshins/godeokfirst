...
export default function StatsSection() {
  const stats = [
    { number: '80', unit: '%', label: '成人牙龈疾病患病率', source: '韩国牙周病学会 2024' },
    { number: '14', unit: '% 减少', label: '洁牙后心血管疾病减少', source: '盆唐首尔大学医院 2024' },
    { number: '6', unit: '倍', label: '牙周炎时血糖控制失败', source: '天主教大学医学院 2023' }
  ];
        ...
        <div className="box-border flex flex-col gap-[60px] items-start px-4 py-[60px] relative w-full max-w-[430px]">
          ...
          <div className="flex flex-col gap-4 items-center justify-center relative shrink-0 w-full z-10">
            <div className="flex flex-col gap-3 items-center justify-center relative shrink-0 w-full">
              ...
              {/* Title */}
              <h2 className="font-['Pretendard,_sans-serif'] font-bold leading-[1.35] text-[30px] text-center text-white tracking-[-0.64px] w-full">
                牙龈疾病，<br />
                不仅仅是口腔问题
              </h2>

              {/* Subtitle */}
              <p className="font-['Pretendard,_sans-serif'] font-semibold leading-[1.4] text-[#e9ebf1] text-base text-center tracking-[-0.32px] w-full">
                严重影响全身健康的<br />
                牙龈疾病危险性
              </p>
            </div>
              ...
              {stats.map((stat, index) => (
                <div key={index} className="bg-white box-border flex flex-col gap-4 items-center px-4 py-10 relative rounded-2xl shrink-0 w-full">
                  <div className="flex gap-2 items-baseline justify-center relative shrink-0 w-full">
                    <p
                      className="font-['Pretendard,_sans-serif'] font-bold leading-none text-[72px] text-center text-nowrap tracking-[-2.88px]"
                      style={{
                    ...
                    <p className="font-['Pretendard,_sans-serif'] font-bold leading-none text-[#006aff] text-2xl text-center tracking-[-0.48px]">
                      {stat.unit}
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 items-start leading-[1.4] not-italic relative shrink-0 text-center w-full">
                    <p className="font-['Pretendard,_sans-serif'] font-bold text-[#37373e] text-[22px] tracking-[-0.44px] w-full">
                      {stat.label}
                    </p>
                    <p className="font-['Pretendard,_sans-serif'] font-medium text-[#9298a6] text-[14px] tracking-[-0.28px] w-full">
                      {stat.source}
                    </p>
                  </div>
                </div>
